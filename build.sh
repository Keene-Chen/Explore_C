...
-h | --help | h | help)
    ...
    ;;
c | cls | clean | clear)
    echo_info "清理生成的文件和目录..."
    rm -rf bin cmake-build cmake-ninja cmake-vscode
    echo_success "清理完成！"
    exit 0
    ;;
n | ninja | Ninja)
...
if [ $? -eq 0 ]; then
    ...
    echo_info ">>> 使用 ${BUILD_TOOL} 开始构建... >>>"
    cmake --build ${BUILD_DIR} --parallel $(nproc)
else
    echo_error "CMake 配置失败, 请检查您的环境和 CMakeLists.txt 文件"
    show_help
    exit 1
fi
